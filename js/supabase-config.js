...
class SupabaseSync {
  ...
  enableRealtime(onDataChange) {
    ...
    // 监听 dingdang_records 表的变化（只监听INSERT和UPDATE，忽略DELETE）
    const recordsChannel = supabaseClient
      .channel('dingdang-records-changes')
      .on('postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'dingdang_records' },
          async (payload) => {
            console.log('📥 检测到 dingdang_records 新增:', payload.eventType);
            await debouncedSync('records');
          })
      .on('postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'dingdang_records' },
          async (payload) => {
            console.log('📥 检测到 dingdang_records 更新:', payload.eventType);
            await debouncedSync('records');
          })
      .subscribe();

    // 监听 dingdang_tasks 表的变化（只监听INSERT和UPDATE）
    const tasksChannel = supabaseClient
      .channel('dingdang-tasks-changes')
      .on('postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'dingdang_tasks' },
          async (payload) => {
            console.log('📥 检测到 dingdang_tasks 新增:', payload.eventType);
            await debouncedSync('tasks');
          })
      .on('postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'dingdang_tasks' },
          async (payload) => {
            console.log('📥 检测到 dingdang_tasks 更新:', payload.eventType);
            await debouncedSync('tasks');
          })
      .subscribe();

    // 监听 dingdang_gifts 表的变化（只监听INSERT和UPDATE）
    const giftsChannel = supabaseClient
      .channel('dingdang-gifts-changes')
      .on('postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'dingdang_gifts' },
          async (payload) => {
            console.log('📥 检测到 dingdang_gifts 新增:', payload.eventType);
            await debouncedSync('gifts');
          })
      .on('postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'dingdang_gifts' },
          async (payload) => {
            console.log('📥 检测到 dingdang_gifts 更新:', payload.eventType);
            await debouncedSync('gifts');
          })
      .subscribe();

    // 监听 dingdang_requests 表的变化（只监听INSERT和UPDATE）
    const requestsChannel = supabaseClient
      .channel('dingdang-requests-changes')
      .on('postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'dingdang_requests' },
          async (payload) => {
            console.log('📥 检测到 dingdang_requests 新增:', payload.eventType);
            await debouncedSync('requests');
          })
      .on('postgres_changes',
          { event: 'UPDATE', schema: 'public', table: 'dingdang_requests' },
          async (payload) => {
            console.log('📥 检测到 dingdang_requests 更新:', payload.eventType);
            await debouncedSync('requests');
          })
      .subscribe();
  ...
  async migrateLocalToCloud() {
    ...
    try {
      ...
      // 迁移任务
      const localTasks = JSON.parse(localStorage.getItem('dingdang_tasks') || '[]');
      ...
      // 迁移礼物
      const localGifts = JSON.parse(localStorage.getItem('dingdang_gifts') || '[]');
      ...
      // 迁移记录
      const localRecords = JSON.parse(localStorage.getItem('dingdang_records') || '[]');
      ...
      // 迁移兑换申请
      const localRequests = JSON.parse(localStorage.getItem('dingdang_requests') || '[]');
